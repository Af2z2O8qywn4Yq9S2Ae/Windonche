...
test('contrastes connus et validation des deux palettes', () => {
  assert.equal(contrast('#000000', '#ffffff'), 21);
  assert.equal(contrast('#808080', '#808080'), 1);
  for (const theme of Object.values(THEMES)) assert.doesNotThrow(() => validateContrast(theme));
...
test('migration des anciennes clés et rejet des noms de thème hérités du prototype', () => {
  ...
  assert.deepEqual(engine.state, { theme: '95', enabled: false, compact: true });
